...
%precedence LOWEST_PRECEDENCE //%left
//%left VALUE_MACRO
%left OR
%left AND

%precedence reduceAttrib //%left
...
setActiveToExpected
    : %empty            {   parser->setCurrentToExpected(); $$.clear(); }
    ;
...
moduleOptions
    : %empty                                                                %prec LOWEST_PRECEDENCE     // Ensure that '(' gets shifted instead of causing a s/r error
...
beginConditionalScope
    : %empty            {   parser->enterScope(false); $$.clear(); }
    ;
...
optBuildFlags
    : %empty            { $$.setNullExpr(); $$.clearPosition(); }
...
endRowsGroup
    : %empty            {   $$.setExpr(parser->endRowsScope()); }
    ;

endSimpleFilter
    : %empty            {   parser->popTopScope(); $$.clear(); }
    ;
...
startStoredAttrs: %empty{ parser->enableAttributes(STORED); $$.clear(); } ;
