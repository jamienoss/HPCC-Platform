...
void Connection::handleLockOnGet(ICodeContext * ctx, const char * key, MemoryAttr * retVal, const char * password)
{
    ...
    //Check if returned value is locked and that the it is non-empty (but in a non-existent  key fashion).
    if (strncmp(reply->query()->str, REDIS_LOCK_PREFIX, strlen(REDIS_LOCK_PREFIX)) != 0 && reply->query()->type != REDIS_REPLY_NIL)
    {
    ...
    else
    {
        ...
        if (nakedReply->type == REDIS_REPLY_ARRAY && strcmp("message", nakedReply->element[0]->str) == 0)
        {
            ...
            if (message && *message)
            {
                retVal->set(nakedReply->element[2]->len, message);//return the published value rather than another (WATCHed) GET.
                return;
            }
            VStringBuffer msg("Redis Plugin: ERROR - the requested key '%s' does not exist on database %" I64F "u", key, database);
            rtlFail(0, msg.str());
