...
**Installation and Dependencies** for requirement). In addition, the following system environment setting `HPCC_REDIS_PLUGIN_CONNECTION_CACHING_LEVEL` can be set as
the following: `0` to force any & all caching OFF, `1` to allow caching as default
and as a function parameter, and `2` to force any & all caching ON (not bound
checked).
